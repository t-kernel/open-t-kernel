...
SYSCALL ER _tk_ras_tex( ID tskid, INT texcd )
{
	...
	if ( rastex ) {
		/* Execute self-break function
		   if task exception occurred while at break function is in execution */
		if ( ctxtsk->exectex != 0 ) {
			call_brkhdr(ctxtsk);	/* Execute break function */
		}
	}
...
EXPORT ER resource_group_initialize( void )
{
	...
	resid_bitmap = Icalloc(1, (UINT)((NUM_RESID + 31) / 32));
	if ( resid_bitmap == NULL ) {
		return E_NOMEM;
	}

	/* System resource always exist */
	BitSet(resid_bitmap, resid_to_index(SYS_RESID));
...
SYSCALL ID _tk_cre_res( void )
{
	...
	BEGIN_CRITICAL_SECTION;
	n = BitSearch0_w(resid_bitmap, 0, NUM_RESID);
	if ( n < 0 ) {
		ercd = E_LIMIT;
	} else {
		BitSet(resid_bitmap, n);
...
SYSCALL ER _tk_del_res( ID resid )
{
	...
	BEGIN_CRITICAL_SECTION;
	idx = resid_to_index(resid);
	if ( !BitTest(resid_bitmap, idx) ) {
		ercd = E_NOEXS;
	} else {
		BitClr(resid_bitmap, idx);
	}
	END_CRITICAL_SECTION;
...
SYSCALL ER _tk_get_res( ID resid, ID ssid, void **p_resblk )
{
	...
	BEGIN_CRITICAL_SECTION;
	if ( !BitTest(resid_bitmap, idx) || ssycb->svchdr == no_support ) {
		ercd = E_NOEXS;
		goto error_exit;
	}
